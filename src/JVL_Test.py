...
class MyFrame(wx.Frame):
    ...
    def InitUI(self):
        ...
        pnl = wx.Panel(self)
        #Devide the main panel vertically to leftPanel and lowerPanel
...
#----------------------------------------------------------------------------------
        ...
        ParamBox = wx.GridBagSizer(3, 7)
        
#------------------------------------------------------------------------------------  
        #Define parameters
        st1 = wx.StaticText(lowerPanel, label='Start_Voltage')
        self.tc1 = wx.TextCtrl(lowerPanel, 1,value="-1.2", size=(55,20))
        ParamBox.Add(st1, pos=(1,0), flag=wx.LEFT, border=15)
        ParamBox.Add(self.tc1, pos=(1,1), flag=wx.LEFT, border=2)
        ...
        self.tc2 = wx.TextCtrl(lowerPanel, -1, value='1.2', size=(55,20))
        ...
        # st3 = wx.StaticText(lowerPanel, label='End')
        # self.tc3 = wx.TextCtrl(lowerPanel, -1, value='1.2', size=(55,20))      
        # ParamBox.Add(st3, pos=(3,0), flag=wx.LEFT, border=15)
        # ParamBox.Add(self.tc3, pos=(3,1), flag=wx.LEFT, border=2)
        # ParamBox.Add(wx.StaticText(lowerPanel, label='V'), pos=(3,2), flag=wx.LEFT, border=1)
        ...
        ParamBox.Add(st4, pos=(1,4), flag=wx.LEFT, border=15)
        ParamBox.Add(self.tc4, pos=(1,5), flag=wx.LEFT, border=2)
        ParamBox.Add(wx.StaticText(lowerPanel, label='ms'), pos=(1,6), flag=wx.LEFT, border=1)
        
        st5 = wx.StaticText(lowerPanel, label='Area')
        self.tc5 = wx.TextCtrl(lowerPanel, -1, value='0.045', size=(55,20))      
        ParamBox.Add(st5, pos=(2,4), flag=wx.LEFT, border=15)
        ParamBox.Add(self.tc5, pos=(2,5), flag=wx.LEFT, border=2)
        ParamBox.Add(wx.StaticText(lowerPanel, label='cm2'), pos=(2,6), flag=wx.LEFT, border=1)
        
        st6 = wx.StaticText(lowerPanel, label='xxxx')
        self.tc6 = wx.TextCtrl(lowerPanel, -1, value='100', size=(55,20))
        ParamBox.Add(st6, pos=(3,4), flag=wx.LEFT, border=15)
        ParamBox.Add(self.tc6, pos=(3,5), flag=wx.LEFT, border=2)
        ParamBox.Add(wx.StaticText(lowerPanel, label='yyyy'), pos=(3,6), flag=wx.LEFT, border=1)
...
#----------------------------------------------------------------------------------------------------
        ...
        stop_Btn = wx.Button(lowerPanel, label='STOP', size=(80, 70))
...
#-------------------------------------------------------------------------------------------        
        ...
        stability_para = wx.StaticText(lowerPanel, label='Stability Parameters')
        ...
        st11 = wx.StaticText(lowerPanel, label='Period')
        self.tc11 = wx.TextCtrl(lowerPanel, 1, value="200", size=(55,20))
        ...
        StabilityParamBox.Add(wx.StaticText(lowerPanel, label='hr'), pos=(0,2), flag=wx.LEFT, border=1)
        
        st12 = wx.StaticText(lowerPanel, label='Interval')
        self.tc12 = wx.TextCtrl(lowerPanel, 1, value="10", size=(55,20))
        StabilityParamBox.Add(st12, pos=(1,0), flag=wx.LEFT, border=15)
        StabilityParamBox.Add(self.tc12, pos=(1,1), flag=wx.LEFT, border=2)
        StabilityParamBox.Add(wx.StaticText(lowerPanel, label='min'), pos=(1,2), flag=wx.LEFT, border=1)

#--------------------------------------------------------------------------------------
        #Checking current density of device1-pixel2
        hbox = wx.BoxSizer(wx.HORIZONTAL)        
        st8 = wx.StaticText(lowerPanel, label='Jsc of Device 1 :')
        hbox.Add(st8, flag=wx.LEFT|wx.ALIGN_CENTER, border=5)
        self.c_density = wx.StaticText(lowerPanel, label='')
        hbox.Add(self.c_density, flag=wx.LEFT|wx.ALIGN_LEFT, border=5)

#------------------------------------------------------------------------------------
        #Construct all components in the lowerPanel screen        
        ...
        vbox1.Add(ParamBox)
        vbox1.Add(wx.StaticLine(lowerPanel), 0, wx.ALL|wx.EXPAND, 10)
...
#---------------------------------------------------------------------------------
        ...
        hBox.Add(lowerPanel, 0, wx.EXPAND)
...
#--------------------------------------------------------------------------------------        
    def OnSingleMeasurement(self, event):   #Current measurement function for Jsc display
        ...
        ser_keithley = serial.Serial(port="COM3", baudrate=38400, parity=serial.PARITY_NONE, bytesize=8, stopbits=1)
...
#----------------------------------------------------------------------------------
    def OnMeasurement(self, event):             #Main I-V measurement function
        ...
        ser_keithley.write(b':SENS:CURR:PROT 0.5\r')     #Cmpl current in Ampere
        # ser_keithley.write(b':SENS:CURR:NPLC 0.1\r')     #AC noise integrating time
        ser_keithley.write(b':SENS:CURR:NPLC %f\r' %NPLC)     #AC noise integrating time
...
#-----------------------------------------------------------------------------------------
    def chamber_select(self, event):        #4- or 9-Device chamber selection function
        if not self.cb_4d.IsChecked() and not self.cb_9d.IsChecked():
            self.cb_4d.SetValue(False)
            self.cb_9d.SetValue(True)       #9-Device chamber is a default one
            pixel = [2,3]
            return pixel
...
#-----------------------------------------------------------------------------------------
    def OnMultiMeasurement(self, event):        #Activated by 'J-V Measure' Button
        ...
        suffix = self.mode_select(event)[0]    #suffix for saved file name 
            ...
            for j in pixel:
                ...
                for k in range(len(voltage) - 1):       #Find Jsc
                    if voltage[k] == 0:
                        ...
                        Jsc = (current[k] + ratio * (current[k+1] - current[k]))/area
                ...
                self.matplotlibhrapg.drawData(value_list, device_num, pixel_num, flag, area)
...
######################################################################
    ...
    def pixel_on(self, device_num, pixel_num):
        ...
        #serial port selection for each switch board (COM4: TopBoard, COM5: BottomBoard)
        ser_switch = [serial.Serial("COM4", baudrate=9600, bytesize=8 ,stopbits=1), serial.Serial("COM5", baudrate=9600, bytesize=8 ,stopbits=1)]
...
#        self.drawData()
    ...
    def drawData(self, value_list, device_num, pixel_num, flag, area):
